...
- `knitr::opts_knit$set(root.dir = ".")`: sets the working directory. By default, the working directory for R code chunks is the directory that contains the Rmd document, so basically `knitr::opts_knit$set(root.dir = ".")` does nothing here. But it might be interesting for you to set another working directory depending on your files organization. We recommend **ALWAYS** set the working directory relatively to your `.Rmd` file! Thus, if you share your research compendium with someone else, this someone will be able to knit your `.Rmd` file without changing anlything!
...
The code chunck options were : "r pressure1, echo=FALSE" <br/>
...
3. the `echo = FALSE` parameter prevent printing of the R code that generated the plot.
...
The code chunck options were : "r pressure2, echo=TRUE, fig.align='center', fig.width=4, fig.height=3, fig.cap="Figure 2: my nicest plot ever", fig.path="figures""

Here we have 7 elements: 

3. the `echo = TRUE` parameter now prints of the R code that generated the plot.
...
The code chunck options were : "r pressure3, echo=TRUE, error=TRUE, include=TRUE, warning=TRUE"
...
5. the `include = TRUE` parameter now prints of the code = redundant with echo
6. the `warning = TRUE` prints the warning messages, in this case the ggplot2 version warning
...
The code chunck options were : "r cache example, fig.width=3, fig.height=3, cache=TRUE"
Here we have 6 elements: 

5. the `cache=TRUE` parameter now **caches the result of this code chunks** 
